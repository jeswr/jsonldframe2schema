...
@pytest.mark.parametrize("test_name,frame_file,schema_file", TEST_CASES, ids=[tc[0] for tc in TEST_CASES])
...
class TestFileValidity:
    ...
    def test_schema_has_schema_keyword(self, test_name, frame_file, schema_file):
        ...
        assert "$schema" in schema, f"Expected schema in {schema_file.name} should have $schema keyword"
